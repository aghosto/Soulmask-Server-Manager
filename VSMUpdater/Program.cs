...
{
    Console.WriteLine("SoulMaskServerManager正在运行中，请在更新之前退出VSM。");
...
if (!File.Exists(workingDir + @"\SoulMaskServerManager.exe"))
{
    Console.WriteLine("在根目录中找不到‘SoulMaskServerManager.exe’。请确保应用程序已正确安装。");
    Console.ReadKey();
    Environment.Exit(2);
}
...
if (File.Exists(workingDir + @"\SSMSettings.json"));
    File.Copy(workingDir + @"\SSMSettings.json", workingDir + @"\Backups\VSMSettings.bak", true);
...
Process.Start("SoulMaskServerManager.exe");
